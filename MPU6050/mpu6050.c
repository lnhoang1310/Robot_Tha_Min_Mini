...
void Calib(MPU6050_t* mpu){
	double sum_gz = 0;
	double sum_gx = 0;
	double sum_gy = 0;
	for(uint16_t i=0; i<NUM_SAMPLE_CALIB; i++){
	...
	Gz_Error = sum_gz / NUM_SAMPLE_CALIB;
}
...
MPU6050_Status MPU6050_Read_All_Fast(MPU6050_t *mpu) {
	...
	mpu->Gz = LowPassFilter(&gz_filter, mpu->Gyro_Z_RAW / 131.0f);
    ...
    mpu->Yaw += ((robot.status == ROBOT_RUN) ? ((mpu->Gz - Gz_Error) * dt) : 0) * PI / 180.0f;
//	mpu->Yaw += (mpu->Gz - Gz_Error) * dt;
	return MPU6050_OK;
}
